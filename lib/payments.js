...
export function orderTotalsFromSettings(items, settings, coupon, paymentMethod) {
  // Shipping configuration - supports both old and new field names
  let shippingCost = settings?.shipping?.shippingCost ?? settings?.deliveryFee ?? 250;
  let freeShippingThreshold = settings?.shipping?.freeShippingThreshold ?? settings?.freeDeliveryThreshold ?? 5000;
  ...
  // Determine shipping
  if (
    settings?.shipping?.freeShipping === true ||
    settings?.shipping?.freeShippingGlobal === true
  ) {
    shippingCost = 0;
    shippingFree = true;
  } else if (
  ...
  const shippingLabel = shippingFree ? "Free shipping" : `Rs ${shippingCost} shipping`;
  ...
  let tax = 0;
  let taxLabel = settings?.tax?.label || "Tax";
  let taxInclusive = settings?.tax?.inclusive === true;

  if (settings?.tax?.enabled === true && settings?.tax?.rate > 0) {
    const taxRate = Number(settings.tax.rate) || 0;
    if (taxInclusive) {
      // Tax is already included in the price — calculate what portion it is
      tax = subtotal - (subtotal / (1 + taxRate / 100));
    } else {
      // Tax is added on top of subtotal and shipping, minus discount
      const taxBase = subtotal + shippingCost - discount;
      tax = (taxBase * taxRate) / 100;
    }
    tax = Math.round(tax);
  }

  // COD fee - only when COD is selected and enabled
  const codFee = 
    paymentMethod === "COD" && settings?.shipping?.codEnabled !== false
      ? (settings?.shipping?.codFee || 0)
      : 0;

  // Calculate total
  let total;
  if (taxInclusive) {
    // Tax is already in subtotal, so don't add it again
    total = subtotal + shippingCost + codFee - discount;
  } else {
    // Tax is added on top
    total = subtotal + shippingCost + tax + codFee - discount;
  }
