...
export function checkoutConfigsFromSettings(settings) {
  if (!settings) {
    return {
      shipping: { threshold: 0, rate: 0, freeShippingGlobal: false, codEnabled: true, codFee: 0 },
  ...
  return {
    shipping: {
      ...
      codEnabled: Boolean(settings.shipping?.codEnabled) ?? true,
      codFee: Number(settings.shipping?.codFee) || 0,
    },
    tax: {
